...
function feature_selection_overall_demand()
    # The regressors that are important
    stream = loadStream(stream_name="electricity-load-nyiso-overall.json",
    ...
    optional_regressors = [
        ["heat_index", Set(filter(x -> contains(String(x), "heat_index"), colnames(stream[1])))],
...
#        ["maximum_wind_speed", Set(filter(x -> contains(String(x), "maximum_wind_speed"), colnames(stream[1])))],
...
#        ["minimum_wind_speed", Set(filter(x -> contains(String(x), "minimum_wind_speed"), colnames(stream[1])))],
        ["wind_components", Set(filter(x -> contains(String(x), "wind_component"), colnames(stream[1])))],
    ]
    compareFeaturePerformance(;stream=stream,
    always_regressors=always_regressors,
    optional_regressors=optional_regressors,
    combo_lengths=3:3,
    trial_count=2,
    learning_rate=0.001)
...
function determine_distributions_for_streams(stream_starts_with, lag_interval)
    streams = sort(collect(filter(x -> startswith(x, stream_starts_with), keys(Microprediction.get_sponsors(Microprediction.Config())))))



    results = try_stream.(streams)

    return Dict(zip(streams, results))
